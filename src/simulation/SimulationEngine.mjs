...
export class SimulationEngine{
  ...
  setDimensionMode(mode){this.env.dimensionMode=mode==='2D'?'2D':'3D';this.grid.setDimensionMode(this.env.dimensionMode);if(this.env.dimensionMode==='2D'){for(let i=0;i<this.ps.count;i++){this.ps.z[i]=this.ps.pz[i]=0;this.ps.vz[i]=0;}this.gravity.z=0;}this.grid.configure(this.env.box);}
  ...
  emit(materialId,position,count=16,temperature,spread=1){const m=this.materials.get(materialId);if(!m)return 0;let added=0;const planar=this.env.dimensionMode==='2D',phase=temperature!==undefined&&temperature>m.boilingTemperature+m.phaseTransitionHysteresis?Phase.GAS:(temperature!==undefined&&temperature<m.meltingTemperature-m.phaseTransitionHysteresis?Phase.SOLID:Phase.LIQUID);for(let n=0;n<count;n++){const a=n*2.399963,r=(.05+.035*Math.sqrt(n))*spread,x=position.x+Math.cos(a)*r,y=planar?position.y+Math.sin(a)*r:position.y+(n%4)*.035*spread,z=planar?0:position.z+Math.sin(a)*r,i=this.ps.add({x,y,z,mass:1,temperature},m,phase);if(i>=0)added++;else break;}return added;}
  ...
  sanitize(){const planar=this.env.dimensionMode==='2D';for(let i=0;i<this.ps.count;i++){for(const key of ['x','y','z','vx','vy','vz','temperature','density'])if(!Number.isFinite(this.ps[key][i]))this.ps[key][i]=0;this.ps.temperature[i]=clamp(this.ps.temperature[i],0,1.2);this.ps.density[i]=clamp(this.ps.density[i],.04,2.5);if(planar){this.ps.z[i]=this.ps.pz[i]=0;this.ps.vz[i]=0;}}this.ps.clampToBox(this.env.box);}
