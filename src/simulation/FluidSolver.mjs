...
export class FluidSolver {
  ...
  step(ps,grid,gravity,env,rules,dt,iterations=2){
    ...
    for(let it=0;it<iterations;it++){
      ...
      for(let i=0;i<ps.count;i++){
        ...
        grid.forEachNeighbor(ps,i,this.neighborRadius,(j)=>{
          const dx=ps.x[j]-ps.x[i],dy=ps.y[j]-ps.y[i],dz=ps.z[j]-ps.z[i],d=Math.hypot(dx,dy,dz);if(d<=1e-6||d>this.neighborRadius)return;
    ...
    for(let i=0;i<ps.count;i++){
      const rx=(ps.x[i]-ps.px[i])/dt,ry=(ps.y[i]-ps.py[i])/dt,rz=(ps.z[i]-ps.pz[i])/dt;
      if(ps.phase[i]===Phase.SOLID){
        ...
        ps.vz[i]=ps.vz[i]*(1-constraintVelocityShare)+rz*constraintVelocityShare;
      }else{
        const freezing=ps.phase[i]===Phase.LIQUID&&ps.temperature[i]<ps.meltingTemperature[i]-ps.phaseHysteresis[i]?clamp(ps.phaseProgress[i],0,1):0;
        ...
        ps.vz[i]=rz*(1-physicalShare)+ps.vz[i]*physicalShare;
      }
    }
  }
  collideBox(ps,box){
    ...
    for(let i=0;i<ps.count;i++){
      ...
      if(ps.z[i]<-hz){ps.z[i]=-hz;if(ps.vz[i]<0)ps.vz[i]=-ps.vz[i]*restitution;hitZ=true;}else if(ps.z[i]>hz){ps.z[i]=hz;if(ps.vz[i]>0)ps.vz[i]=-ps.vz[i]*restitution;hitZ=true;}
