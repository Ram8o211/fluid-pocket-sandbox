    ...
    grid.forEachPair(ps,radius,(i,j)=>{
      ...
      if(potentialDelta>thermalThreshold){
        ...
        if(Math.abs(energy)>1e-6){
          ps.temperature[i]=clamp(ps.temperature[i]+energy/Math.max(.15,ps.heatCapacity[i]),0,1.2);ps.temperature[j]=clamp(ps.temperature[j]+energy/Math.max(.15,ps.heatCapacity[j]),0,1.2);thermalEvent=Math.sign(energy)*Math.min(1,Math.abs(energy)*24);
        }
