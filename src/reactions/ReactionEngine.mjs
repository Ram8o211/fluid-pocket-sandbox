...
export class ReactionEngine {
  ...
  step(ps,grid,rules,dt,onFuse=null){
    ...
    for(let i=0;i<ps.count;i++) grid.forEachNeighbor(ps,i,radius,(j)=>{
      ...
      // Thermochemical reactions are contact-local and threshold based, independently
      // from whether the two substances are ultimately miscible.
      const thermalThreshold=clamp(rules.thermalReactionThreshold??.72,0,1.95);
      if(potentialDelta>thermalThreshold){
        const excess=saturate((potentialDelta-thermalThreshold)/Math.max(.05,2-thermalThreshold));
        const heatBias=clamp((ps.reactionHeat[i]+ps.reactionHeat[j])*.5,-1,1);
        const thermalStrength=Math.max(0,rules.thermalReactionStrength??1.15),energy=heatBias*excess*contact*thermalStrength*dt*.18;
        if(Math.abs(energy)>1e-6){
          ps.temperature[i]=clamp(ps.temperature[i]+energy/Math.max(.15,ps.heatCapacity[i]),0,1.2);
          ps.temperature[j]=clamp(ps.temperature[j]+energy/Math.max(.15,ps.heatCapacity[j]),0,1.2);
          thermalEvent=Math.sign(energy)*Math.min(1,Math.abs(energy)*24);
        }
        ...
        ps.reactionPotential[i]+=(mean-ps.reactionPotential[i])*consume;ps.reactionPotential[j]+=(mean-ps.reactionPotential[j])*consume;
        visual=Math.max(visual,excess*contact);reacted=true;
      }
