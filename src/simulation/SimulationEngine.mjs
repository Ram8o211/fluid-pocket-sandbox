...
export class SimulationEngine{
  ...
  step(dt){
    if(this.paused)return;dt=clamp(dt,1/120,1/24);this.time+=dt;this.stepIndex++;const planar=this.env.dimensionMode==='2D',profile=performanceProfileFor(this.ps.count,this.env.dimensionMode);this.performance=profile;
    const fluidIterations=Math.min(this.iterations,profile.fluidIterations);
    ...
    let changes=0,hasSolid=false;
    for(let i=0;i<this.ps.count;i++){
      const change=updatePhaseParticle(this.ps,i,dt);if(change){changes++;if(change.from===Phase.SOLID&&change.to===Phase.LIQUID)this.solid.onMelt(this.ps,i);else if(change.to===Phase.SOLID)this.solid.initializeParticle(this.ps,i,planar,this.grid);}if(this.ps.phase[i]===Phase.SOLID)hasSolid=true;
    }
    let positionsDirty=false;
    if(hasSolid){this.solid.step(this.ps,this.grid,this.env,dt);this.fluid.collideBox(this.ps,this.env.box,planar);positionsDirty=true;}
    if(this.molds.molds.length){this.molds.collide(this.ps,planar);positionsDirty=true;}
    if(positionsDirty)this.grid.rebuild(this.ps,this.env.box);
  ...
  sanitize(clampBox=true){
    const ps=this.ps,planar=this.env.dimensionMode==='2D',x=ps.x,y=ps.y,z=ps.z,vx=ps.vx,vy=ps.vy,vz=ps.vz,temp=ps.temperature,density=ps.density,potential=ps.electricPotential;
    for(let i=0;i<ps.count;i++){
      if(!Number.isFinite(x[i]))x[i]=0;if(!Number.isFinite(y[i]))y[i]=0;if(!Number.isFinite(z[i]))z[i]=0;if(!Number.isFinite(vx[i]))vx[i]=0;if(!Number.isFinite(vy[i]))vy[i]=0;if(!Number.isFinite(vz[i]))vz[i]=0;
      if(!Number.isFinite(temp[i]))temp[i]=0;if(!Number.isFinite(density[i]))density[i]=0;if(!Number.isFinite(potential[i]))potential[i]=0;
      temp[i]=clamp(temp[i],0,1.2);density[i]=clamp(density[i],.04,2.5);potential[i]=clamp(potential[i],-1.5,1.5);if(planar){z[i]=ps.pz[i]=0;vz[i]=0;}
    }
    if(clampBox)ps.clampToBox(this.env.box);
  }
